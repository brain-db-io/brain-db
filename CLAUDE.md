...
**One read pipeline:** `RECALL` and `QUERY` fan out to the three always-wired retrievers (semantic / lexical / graph) and fuse ranks via **RRF** (`k=60`), then apply the filter chain (type / temporal / confidence / tombstone / supersession). An optional cross-encoder rerank runs when `request.rerank == true` and the operator has not opted out (`config.rerank.enabled = true` at shard spawn).
