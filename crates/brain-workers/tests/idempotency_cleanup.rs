...
#[tokio::test]
async fn young_entries_are_kept() {
    let fix = build_fixture();
    let now = now_unix_nanos();
...
#[tokio::test]
async fn mixed_entries_only_expired_removed() {
    let fix = build_fixture();
    let now = now_unix_nanos();
    for i in 1..=3u8 {
...
#[tokio::test]
async fn multi_cycle_convergence() {
    let fix = build_fixture();
    let now = now_unix_nanos();
...
#[tokio::test]
async fn empty_table_cycle_is_noop() {
    let fix = build_fixture();
    let worker = IdempotencyCleanupWorker::new();
    let processed = run_one(&worker, fix.ctx).await.unwrap();
    assert_eq!(processed, 0);
...
#[tokio::test]
async fn batch_size_zero_returns_zero() {
    let fix = build_fixture();
    let now = now_unix_nanos();
    for i in 1..=3u8 {
        seed_entry(&fix.metadata, i, now - 30 * HOUR_NS);
