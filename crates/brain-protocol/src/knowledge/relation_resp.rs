//! Relation-op response payloads. Spec §28/07.

use rkyv::{Archive, Deserialize, Serialize};

use brain_core::knowledge::{Relation, RelationType};
...
// ---------------------------------------------------------------------------
// RelationView — read-side projection. Spec §28/07 §2.2.
// ---------------------------------------------------------------------------

/// Wire-domain projection of `brain_core::knowledge::Relation`.
...
pub enum RelationWireError {
    #[error("RELATION evidence overflow not supported in v1 (spec §20/05 §3)")]
...
/// Reply to `RELATION_CREATE` (`0x01D0`). Spec §28/07 §3.2.
...
/// Reply to `RELATION_GET` (`0x01D1`). Spec §28/07 §4.
#[derive(Archive, Serialize, Deserialize, Clone, Debug, PartialEq)]
#[archive(check_bytes)]
#[archive_attr(derive(Debug))]
...
/// Reply to `RELATION_SUPERSEDE` (`0x01D2`). Spec §28/07 §5.
#[derive(Archive, Serialize, Deserialize, Clone, Copy, Debug, PartialEq, Eq)]
#[archive(check_bytes)]
#[archive_attr(derive(Debug))]
...
/// Reply to `RELATION_TOMBSTONE` (`0x01D3`). Spec §28/07 §6.
#[derive(Archive, Serialize, Deserialize, Clone, Copy, Debug, PartialEq, Eq)]
#[archive(check_bytes)]
#[archive_attr(derive(Debug))]
...
/// carrying `Vec<TraversalPathWire>`. Spec §28/07 §9.2's per-frame
