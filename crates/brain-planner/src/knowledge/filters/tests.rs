...
use crate::knowledge::fusion::{FusedItem, RetrieverContribution};
use crate::knowledge::router::{Retriever, TimeRange};
...
fn intern_predicate(metadata: &mut MetadataDb, namespace: &str, name: &str) -> PredicateId {
    let wtxn = metadata.write_txn().expect("wtxn");
    let id = predicate_intern(&wtxn, namespace, name, None, 0, 1, "", 0).expect("predicate");
    wtxn.commit().expect("commit");
    id
}
...
fn statement_kind_and_predicate_filter() {
    ...
    let p_lives = intern_predicate(&mut metadata, "brain", "lives_in");
    let p_likes = intern_predicate(&mut metadata, "brain", "likes");
...
fn time_filter_for_event_statement_uses_event_at() {
    ...
    let type_id = ensure_person_type(&mut metadata);
    let alice = put_entity(&mut metadata, "Alice", type_id);
    let p = intern_predicate(&mut metadata, "brain", "happened");
...
fn confidence_filter_for_statement() {
    ...
    let type_id = ensure_person_type(&mut metadata);
    let alice = put_entity(&mut metadata, "Alice", type_id);
    let p = intern_predicate(&mut metadata, "brain", "lives_in");
