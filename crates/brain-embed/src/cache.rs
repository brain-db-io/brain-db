...
//!
//! `embed_batch` is a pure passthrough — spec §11 names "cues" (single
...
struct CachedEmbedding {
    ...
    fingerprint: [u8; 16],
    #[allow(dead_code)] // spec §1 mandates the field; reads land in Phase 8+
    inserted_at: Instant,
}
...
impl<D: Dispatcher> CachingDispatcher<D> {
    ...
    /// Wrap `inner` with the default capacity (10 000 per spec §5).
    #[must_use]
...
impl<D: Dispatcher> Dispatcher for CachingDispatcher<D> {
    fn embed(&self, text: &str) -> Result<[f32; VECTOR_DIM], EmbedError> {
        // Disabled cache: pure passthrough. Spec §13.
    ...
    /// Pure passthrough; cache covers cues (single texts) per spec §11.
