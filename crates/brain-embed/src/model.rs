...
//!
//! See:
//! - `spec/04_embedding_layer/01_model_choice.md` — BGE-small properties.
//! - `spec/04_embedding_layer/03_inference.md` §9 — the load sequence.
//! - `spec/04_embedding_layer/03_inference.md` §11 — safetensors only
//!   (we tighten to "refuse pickle outright" per SD-5.1-1).
//! - `spec/04_embedding_layer/07_fingerprinting.md` §3 — the fingerprint
...
/// The output dimensionality for v1 (BGE-small-en-v1.5; spec §01).
...
impl ModelHandle {
    /// Load the model directory per spec §03 §9's six-step sequence:
    ...
    pub fn load(config: &EmbedderConfig) -> Result<Self, EmbedError> {
        ...
        // 7. Compute fingerprint per spec §07 §3.
        ...
        // 9. Warm-up inferences (spec §03 §9 step 6).
        let handle = Self {
            model,
            tokenizer,
            fingerprint,
