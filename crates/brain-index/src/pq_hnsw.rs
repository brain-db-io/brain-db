//! `PqHnswIndex<const M: usize>` — HNSW with PQ-compressed payload.
//!
...
//! here — task 25.5 layers it on top via [`crate::pq_hnsw::rerank`]
...
pub struct PqHnswIndex<const M: usize> {
...
impl From<IdMapError> for PqHnswError {
    fn from(e: IdMapError) -> Self {
        match e {
            IdMapError::AlreadyInserted { memory_id_bytes } => {
                PqHnswError::DuplicateMemoryId { memory_id_bytes }
            }
            IdMapError::Exhausted => PqHnswError::IdMapExhausted,
        }
    }
}
...
mod tests {
    ...
    fn insert_then_search_returns_inserted_id() {
        let mut idx = PqHnswIndex::<8>::new(pq_params_default(), arithmetic_codebook::<8>()).unwrap();
    ...
    fn duplicate_insert_rejected() {
        ...
        assert!(matches!(result, Err(PqHnswError::DuplicateMemoryId { .. })));
    }
    ...
    fn nan_vector_rejected_at_insert() {
        ...
        assert!(matches!(
            result,
            Err(PqHnswError::Encode(EncodeError::NotANumber))
        ));
    }
