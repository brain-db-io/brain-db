...
//!
//! Pure noisy-OR formula per spec §19/04 §1:
//!
//! ```text
...
pub struct ConfidenceConfig {
    ...
    /// When `true`, Event evidence does not decay (spec §19/04 §3.3).
    pub event_decay_disabled: bool,
}

impl ConfidenceConfig {
    /// Defaults per spec §19/04 §3.
    #[must_use]
...
/// Aggregate confidence per spec §19/04 §1.
///
/// - Empty evidence → `0.0`.
...
fn exp_decay(age_secs: f32, half_life_seconds: u64) -> f32 {
    ...
    // that needs `exp(-ln(2) * age / half_life)`. Spec §3.1 uses the
