...
The primitives are introduced briefly here, with their conceptual shape and basic latency expectations. Their exact semantics, parameters, and edge cases are specified in [09. Cognitive Operations](../09_cognitive_operations/).
...
The idempotency horizon is configurable (default: 5 minutes). Retries delayed beyond the horizon may be treated as new operations. See [05. Storage: Arena & WAL](../05_storage_arena_wal/) §4.4.2 for the deduplication mechanism.

### 1.3 Salience hint

The optional `salience_hint` lets the agent signal importance. It's a number in [-1, +1]; positive values raise initial salience, negative values lower it. The actual initial salience is computed from a combination of the hint, the embedding's surprise (distance from the centroid of recent memories), and configured constants. See [02. Data Model](../02_data_model/) §6 for the formula.
...
This is intentional: by concentrating mutations into one well-defined primitive, the read path can be lock-free and the substrate can scale reads independently from writes.
...
For small `top_k` (≤ 10), the substrate may emit all results in a single frame. The client SDK presents both cases with the same async-iterator interface.
...
The agent asks the substrate to construct a path from a start state to a goal, using its memory as the world model.

### 3.1 What the substrate does
...
`PLAN` exercises the graph store, the ANN index (for heuristic estimates), and the attractor dynamics. It is the most complex primitive in terms of execution variety, and the planner's strategy choice is most consequential here.
...
The agent presents an observation and asks the substrate why — what stored memories causally explain the observation, what predictions the substrate makes, what evidence supports each conclusion.

### 4.1 What the substrate does
...
Future iterations expand the operator set. The substrate's architecture supports arbitrary inference DAGs; v1 limits the patterns to what we can confidently calibrate.
...
`REASON` is where Vector Symbolic Architectures (VSA) algebra earns its keep. Bind, bundle, and unbind operations let the substrate manipulate compositional representations — "memory A combined with role B" — without losing structure. See [09. Cognitive Operations](../09_cognitive_operations/) §REASON for the algebra.
...
Decay is not a cognitive primitive — the agent doesn't trigger it — but it is part of the substrate's behavior. Documented in [11. Background Workers](../11_background_workers/).
...
The complete handshake is specified in [03. Wire Protocol](../03_wire_protocol/) §10.
...
The agent registers interest in events matching a filter; the substrate pushes notifications when matching memories are encoded or modified.
...
Operations within a transaction carry the `txn_id`. The substrate buffers them; on `TXN_COMMIT`, all operations are applied atomically (all succeed or all fail). On `TXN_ABORT` or connection drop before commit, none of the operations take effect.
...
These require elevated permissions (see [14. Observability + Operations](../14_observability_ops/) for the permission model). They are invoked over the same protocol as cognitive operations, just with different opcodes.
