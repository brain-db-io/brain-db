...
│     │   • Decay, consolidation, HNSW maintenance, snapshot           │
...
**Detail spec:** This document, [`02_background.md`](02_background.md) §6 covers the runtime choice. The connection layer's specifics are in [03. Wire Protocol](../03_wire_protocol/).

### Responsibilities

- TCP accept loop on a configurable port (default `7474`).
- Optional TLS termination via [rustls](https://github.com/rustls/rustls).
- Session establishment: protocol version negotiation, authentication, namespace selection.
...
Within the connection layer, when a frame arrives, it is routed to the core that owns the target shard. This may not be the core that received the connection; if it isn't, the frame is forwarded via a per-core SPSC queue. We accept this one cross-core hop because:
...
**Detail spec:** [04. Embedding Layer](../04_embedding_layer/).
...
The substrate owns embedding. We argued in [`01_problem.md`](01_problem.md) §4.2 that the substrate should take text, not pre-computed vectors, because embedding ownership is what enables:
...
The cost is making the substrate ML-aware. We accept this; it's the layer where this awareness belongs.
...
**Detail spec:** [08. Query Planner + Execution Engine](../08_query_planner/).
...
**Detail spec:** [08. Query Planner + Execution Engine](../08_query_planner/) §3 onward.
...
Multiple readers run concurrently on the same core (via cooperative async multitasking) and may run concurrently with the writer. Reader-writer coordination uses epoch-based reclamation rather than locks. See [10. Concurrency + Epoch Model](../10_concurrency_epochs/).

---

## L5: Storage Layer

**Detail specs:** [05. Storage: Arena & WAL](../05_storage_arena_wal/) and [07. Metadata + Graph Store](../07_metadata_graph/).
...
**Detail spec:** [11. Background Workers](../11_background_workers/).
...
Rebuilds parts of the HNSW index when the topology degrades. ANN indexes get worse over time as nodes are added and (especially) removed; periodic maintenance keeps recall quality high. Heuristics for when to rebuild are in [06. ANN Index](../06_ann_index/).
...
**Detail spec:** [12. Sharding + Clustering](../12_sharding_clustering/).
...
Specified in [12. Sharding + Clustering](../12_sharding_clustering/).

### What's deferred

**Replication** is intentionally deferred from v1. The first version assumes a single replica per shard; loss of a node means loss of its agents until restored from snapshot. This is documented as a known limitation, with replication slated for a future revision (see [`10_open_questions.md`](10_open_questions.md) OQ-2).
...
**Authentication and authorization.** These cut across layers — auth is established at L_1, but every layer respects the authenticated `agent_id`. They're documented in [14. Observability + Operations](../14_observability_ops/).

**Observability (metrics, tracing, logs).** Every layer emits its own observability data; there's no central observability layer. Conventions are in [14. Observability + Operations](../14_observability_ops/).
...
*Continue to [`05_hardware.md`](05_hardware.md) for the hardware envelope.*
