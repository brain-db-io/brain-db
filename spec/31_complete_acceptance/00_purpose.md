# the knowledge layer Acceptance Gate

What "the knowledge layer is done" means. Concrete tests that must pass.

## Functional acceptance

### Substrate-only mode

- [ ] With no schema declared, all substrate (sections 00-16) acceptance tests pass.
- [ ] Substrate-SDK clients (no knowledge-layer types) work normally when no schema is declared.
...
- [ ] ENCODE: P50 ≤ 1 ms, P99 ≤ 10 ms (with pattern extractors enabled).
- [ ] ENCODE + classifier extractors: P50 ≤ 5 ms, P99 ≤ 20 ms.
...
- [ ] Entity resolution (tier 1-2): P50 ≤ 1 ms.
- [ ] Entity resolution (tier 3, embedding): P50 ≤ 10 ms.

### Throughput

- [ ] ENCODE throughput (pattern extractors only): ≥ 5K/s per shard.
...
- [ ] Switching a deployment from "with schema" to "without schema": substrate continues to serve; knowledge-layer data preserved but unused.

## Documentation acceptance

- [ ] Spec sections 00–16 are complete and internally consistent.
- [ ] All file/directory references in the spec resolve.
- [ ] At least one end-to-end tutorial exists, walking from blank deployment to a working hybrid query.
- [ ] Open questions are tagged with rationale and version-target.

## What we explicitly do NOT include in the knowledge layer

(See `30_knowledge_open_questions/`.)
...
These are future versions / v3 work.

## How to run the acceptance suite

```bash
# substrate regression (sections 00-16)
cargo test --workspace --features substrate

# knowledge-layer functional
cargo test --workspace --features knowledge

# performance
cargo bench --workspace --features knowledge

# integration (e2e against a running binary)
