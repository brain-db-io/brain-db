...
> **TL;DR.** Brain's binary protocol over TCP. 32-byte fixed frame header, rkyv-encoded structured payloads, bytemuck-cast raw vector bytes. One unified opcode space: substrate operations at `0x00xx`, typed-graph operations at `0x01xx`. Handshake (HELLO/WELCOME/AUTH/AUTH_OK), streaming for long-running operations, structured error codes, server-side validation of every field. The protocol is what a third-party client implementer needs to talk to Brain.
...
| Referenced by | [05. Operations](../05_operations/00_purpose.md), [06. SDK Design](../06_sdk/00_purpose.md) |
...
- **Payload encoding.** rkyv for structured payloads, bytemuck for raw vector bytes, the rationale for splitting them. ([`02_wire_format.md`](02_wire_format.md))
...
- **SDK ergonomics.** How a Python or Rust client wraps the protocol — defined in [06. SDK Design](../06_sdk/00_purpose.md).
...
The split between this spec and [05. Operations](../05_operations/00_purpose.md) is sharp: this spec is byte-level; that spec is semantic. A client implementer reads both.
...
- [06. SDK Design](../06_sdk/00_purpose.md) — SDKs implement this protocol.
