...
throughput, and recovery behaviour against the spec §16 targets.
...
- p99 RECALL (K=10, no text) ≤ 20 ms — `spec/16_benchmarks_acceptance/02_latency_targets.md` §2.
- p99 ENCODE ≤ 25 ms.
- All other operations: see `spec/16_benchmarks_acceptance/02_latency_targets.md` §2.
- Throughput: see `spec/16_benchmarks_acceptance/03_throughput_targets.md`.
...
spec §16/02 reports. Results land in `baselines-<date>.md` files
under this directory.

Spec §16/13 expects ±10 % run-to-run variance. ±30 % indicates
instability; investigate before reporting numbers.

## Methodology

Per `spec/16_benchmarks_acceptance/07_benchmark_methodology.md`:
...
spec §16/02 reports require Phase 13.2's load generator plus
